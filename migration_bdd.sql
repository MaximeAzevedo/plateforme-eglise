-- Script de migration pour adapter la base de données au nouveau formulaire
...
SELECT 
  ...
  CASE 
    WHEN "Nom" = 'Maxime De Azevedo' THEN 
      CASE 
        WHEN "Dénomination" = 'Catholique' AND "Rue" = '14 rue mozart' THEN 'Église Saint-Pierre' -- Exemple, à adapter
        ELSE "Dénomination" || ' de ' || "Ville" -- Nom générique basé sur la dénomination et ville
      END
    ELSE "Nom"
  END as "Nom",
  ...
  -- Convertir les horaires existants en format JSON
  CASE 
    ...
    WHEN "Heures des cultes et prières"::TEXT LIKE '[%' THEN 
      -- Déjà en format JSON, on garde tel quel
      "Heures des cultes et prières"::JSONB
    ELSE 
...
-- 5. Optionnel : Supprimer l'ancienne table après vérification
-- ATTENTION : Décommentez seulement après avoir vérifié que tout fonctionne !
-- DROP TABLE IF EXISTS "Base de donnée";
